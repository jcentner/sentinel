...
| **Capability tier** | A label (`basic`, `standard`, `advanced`) on a detector indicating the minimum model class it needs. Boundaries are empirical, not parameter-count-based: `basic` = 4B/9B local (same empirical class, scores ~27-32), `standard` = cloud-nano (gpt-5.4-nano, quality jump at ~38-44), `advanced` = cloud-small/frontier. Informational, not enforced. See ADR-011. |
