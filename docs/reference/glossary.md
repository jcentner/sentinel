...
| **Capability tier** | A backward-compatible config hint (`basic`, `standard`, `advanced`) preserved from ADR-011. Superseded as the primary quality mechanism by benchmark-driven model quality (ADR-016). When benchmark data exists for a model×detector pair, it takes precedence over the tier label for prompt strategy selection. `model_capability` in `sentinel.toml` still works as an explicit override. |
...
| **Compatibility matrix** | An empirical quality rating for each model-detector combination, based on real-world benchmarks. Ratings: excellent (<10% FP), good (10–25% FP), fair (25–40% FP), poor (>40% FP). Viewable in `docs/reference/compatibility-matrix.md`, the web UI (`/compatibility`), and `sentinel compatibility` CLI. Source data in `src/sentinel/core/compatibility.py`. |
| **Quality rating** | A benchmark-derived label (excellent/good/fair/poor) indicating how well a specific model performs with a specific detector. Based on false-positive rate measured against real repositories. |
| **Model class** | A category of model by capability: 4B local (e.g. qwen3.5:4b), 9B local (e.g. qwen3.5:9b), cloud nano (e.g. gpt-5.4-nano), cloud frontier (e.g. gpt-5.4). Used in the compatibility matrix instead of specific model names. |
| **Cross-artifact** | A finding category for contradictions discovered by comparing multiple artifact types (code, docstring, tests, docs) for the same symbol. Used by the intent-comparison detector. |
| **Inline-comment-drift** | A detector that flags docstrings whose content no longer matches the function they describe. LLM-assisted, Python-only in v1. |
| **CI/CD drift** | A detector that finds stale path references in GitHub Actions workflows and Dockerfiles (e.g., referencing files that no longer exist). Deterministic. |
